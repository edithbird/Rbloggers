...
TreatPop <- TreatPop %>% group_by(State) %>% summarise(count = sum(Number.of.Prescriptions))
TreatPop <- inner_join(TreatPop, Population)
TreatPop
PercentinTx <- TreatPop %>% group_by(State) %>% mutate(percentReceivingRx = round(count/StatePopul, digits = 3))
PercentinTx
