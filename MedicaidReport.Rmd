...
dim(allDrugsNoNA)
allDrugsNoNA %>% arrange(desc(State))
durationD <-  AK$Number.of.Prescriptions
range(durationD)
breaksD <- seq(0, 45000, by = 500)
breaksD
duration.cutD <- cut(durationD, breaksD, right = F)
...
frequencyTableBind %>% mutate(averageBin = cummean(Bins), midpoint = averageBin+250, Numbermissing = duration.freqD*midpoint*0.581) %>% select(duration.freqD, Bins, midpoint, Numbermissing)
```
ME freq dist

```{r}
durationD <-  ME$Number.of.Prescriptions
range(durationD)
breaksD <- seq(0, 45000, by = 500)
breaksD
duration.cutD <- cut(durationD, breaksD, right = F)
...
frequencyTableBind %>% mutate(averageBin = cummean(Bins), midpoint = averageBin+250, Numbermissing = duration.freqD*midpoint*0.426) %>% select(duration.freqD, Bins, midpoint, Numbermissing)
```
