...
```{r}
allDrugsXX <- allDrugs %>% group_by(State) %>% summarise(meanNA = mean(is.na(Number.of.Prescriptions)))
allDrugsXX
```

```{r}
numberNAValueState <- allDrugs %>% group_by(State) %>% summarise(SumNA = sum(is.na(Number.of.Prescriptions)))
numberNAValueState
```
...
statePortion
ggplot(statePortion, aes(State, portion)) + geom_bar()
bp <- ggplot(statePortion, aes(x="", y=portion, fill=State)) + geom_bar(width = 1, stat = "identity")
...
averageRxPP <- allDrugsnoXX %>% summarise(sum(sumNumber)/sum(StatePopul))
...
library(dplyr)
head(allDrugs)
x <- allDrugs %>% filter(State != "XX") %>% group_by(state = State) %>% summarise(variable = sum(is.na(Number.of.Prescriptions))/prod(dim(Number.of.Prescriptions)))
x

y <- allDrugs  %>% filter(State != "XX") %>% group_by(State) %>% dplyr::summarise(count = n())
y
...
numberNA
head(Binned)
Binned %>% mutate(AddedfromNA = numberNA*percentofWhole)
```
