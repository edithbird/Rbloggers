...
aus2017 <- read.csv("ausopen.csv", header = T, stringsAsFactors = F)
kable(head(aus2017))
sapply(aus2017, class)
names(aus2017)
odds <- aus2017 %>% mutate(Winner = strtrim(Winner, 2))%>% arrange(desc(MaxW))
head(odds, 10)
ggplot(odds, aes(Winner, fill = factor(MaxW))) + geom_bar() + theme(legend.position='null')
odds1 <- odds %>% select(Winner, MaxW)
head(odds1)
hist(odds1$MaxW, breaks = 5)

```

```{r}
ggplot(odds1, aes(MaxW)) + geom_histogram(binwidth = 1)  +  
